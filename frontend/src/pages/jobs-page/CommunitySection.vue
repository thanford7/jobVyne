          ...
          <q-card-section>
            <template v-if="communityUtil.isJobSeeker(member.member_type_bits)">
              ...
              <template v-if="member.job_search_industries">
                <q-chip v-for="industry in member.job_search_industries" icon="business" color="teal-2" title="Industry"
                        dense>
                  {{ industry.name }}
                </q-chip>
              </template>
            ...
            <template v-if="member.job_connections?.length">
              ...
              <q-list separator>
                <q-item v-for="jobConnection in member.job_connections" clickable @click="openUrlInNewTab(jobConnection.job_url)">
                  <q-item-section avatar>
                    <q-avatar rounded>
